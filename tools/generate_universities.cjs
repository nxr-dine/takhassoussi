...
function normalize(s){
  try { return s.normalize('NFKD').replace(/\p{Diacritic}/gu, ''); } catch(e) { return s; }
}
...
let arrText = null;
// Try to capture the exported array with a regex first
...
if (m && m[1]) {
  ...
  arrText = raw.slice(start, end + 1);
}
// Clean trailing semicolons/newlines
arrText = arrText.trim().replace(/;\s*$/,'');
let programs;
try { programs = JSON.parse(arrText); } catch (e) {
...
for(const [etb, info] of byEtb.entries()){
  ...
  const finalAliases = Array.from(aliases).filter(Boolean).map(s=>String(s).trim()).filter((v,i,a)=>a.indexOf(v)===i).slice(0,60);
